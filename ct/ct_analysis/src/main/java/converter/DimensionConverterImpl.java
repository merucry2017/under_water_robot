...
public class DimensionConverterImpl implements DimensionConverter {
    ...
    private void setArguments(PreparedStatement preparedStatement, BaseDimension dimension) {
        ...
        try {
            if(dimension instanceof DateDimension){
                ...
                preparedStatement.setString(++i, dateDimension.getHour());
                preparedStatement.setString(++i, dateDimension.getMinute());
                preparedStatement.setString(++i, dateDimension.getSecond());
            }
        } catch (SQLException e) {
            e.printStackTrace();
        }
    ...
    private String[] getContactDimensionSQL() {
        String query = "SELECT `id_date_dimension` FROM `under_water_date` WHERE `temperature` = ? AND " +
                "`oxygen_capacity` = ? AND `Ammonia = ? AND `pH` = ? ORDER BY `id_date_dimension`;";
        String insert = "INSERT INTO `under_water_date` (`temperature`, `oxygen_capacity`, `Ammonia`, `pH`) VALUES (?, ?, ?, ?);";
    ...
    private String[] getDateDimensionSQL() {
        String query = "SELECT `id` FROM `tb_dimension_date` WHERE `year` = ? AND `month` = ? AND `day` = ? AND `hour` = ? AND `minute` = ? AND `second` = ?ORDER BY `id`;";
        String insert = "INSERT INTO `tb_dimension_date` (`year`, `month`, `day`, `hour`, `minute`, `second`) VALUES (?, ?, ?, ?, ?, ?);";
        return new String[]{query, insert};
    }
    ...
    private String genCacheKey(BaseDimension dimension) {
        ...
        if (dimension instanceof DateDimension) {
            ...
            sb.append("date_dimension")
                    ...
                    .append(dateDimension.getHour())
                    .append(dateDimension.getMinute())
                    .append(dateDimension.getSecond());
        } else if (dimension instanceof ContactDimension) {
            ContactDimension contactDimension = (ContactDimension) dimension;
            sb.append("contact_dimension").append(contactDimension.getTemperature())
                .append(contactDimension.getOxygen_capacity())
                .append(contactDimension.getAmmonia_nitrogen_content())
                .append(contactDimension.getpH());
